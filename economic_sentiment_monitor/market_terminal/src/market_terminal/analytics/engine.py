"""
market_terminal/analytics/engine.py
...
from market_terminal.config.settings import log, WATCHLIST, MACRO_KEYWORDS
...
def compute_sentiment_momentum(key: str) -> dict:
    ...
    for i in range(len(scores) - 2, 0, -1):
        ...
        if match:
            ...
            break
    return {"acceleration": acceleration, "consecutive": consecutive,
            "direction": direction}
...
def detect_regime(scores_cache: dict) -> dict:
    ...
    return {
        ...
        "fg_val": fg_val, "std_sent": round(std_sent, 4),
        "accel": round(accel, 4),
    }
...
def compute_bayesian_signal(ticker: str) -> dict:
    ...
    with get_db() as db:
        ...
        for src in {r[1] for r in recent if r[1]}:
            ...
            for score, ing_ts in src_rows:
                try:
                    target_ts = (
                        datetime.fromisoformat(ing_ts.replace("Z", ""))
                        + timedelta(hours=1)
                    ).isoformat()
                    p = db.execute("""
                        SELECT change_pct FROM market_prices
                    ...
                    pass
            source_acc[src] = round(
                (correct + 1) / (total + 2) if total >= 3 else 0.52, 3
            )
    ...
    for score, src, _ in recent:
        ...
        lo  = (math.log((acc + 1e-9) / (1 - acc + 1e-9)) if score > 0
               else math.log((1 - acc + 1e-9) / (acc + 1e-9)))
    ...
    top_sources = sorted(
        source_acc.items(), key=lambda x: abs(x[1] - 0.5), reverse=True
    )[:2]
