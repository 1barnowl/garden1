...
class ESMApp(App):
    ...
    TabPane       { padding: 0; }
    #status_bar   { height: 1; background: #0d0d0d; color: #4a6a4a; padding: 0 2; }
    #status_bar2  { height: 1; background: #080808; color: #3a3a3a; padding: 0 2; }
    #fg_bar       { height: 1; background: #0a0a00; color: #888800;
                    padding: 0 2; text-style: bold; }
    #regime_bar   { height: 1; background: #050510; color: #5555cc; padding: 0 2; }
    #freshness_bar{ height: 1; background: #000a00; color: #2a5a2a; padding: 0 2; }
    #workspace_bar{ height: 1; background: #050505; color: #555522; padding: 0 2; }
    #chart_view   { height: 1fr; background: #080808; color: #44cc44;
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("", id="freshness_bar")
        yield Static("", id="workspace_bar")
    ...
    def _setup_tables(self):
        ...
        self.query_one("#log_table", DataTable).add_columns(
            "Thread", "Status", "Last OK", "Errs", "Last Error", "Log")

    # ── master UI refresh ─────────────────────────────────────────────────────
    def _ui_refresh(self):
        errs    = CACHE.err_counts
        total_e = sum(errs.values())
        err_s   = ("⚠ ERR " + " ".join(f"{k}:{v}" for k,v in errs.items() if v>0)
                   + "  │  " if total_e else "")
        self.query_one("#status_bar", Static).update(
            f"{err_s}{tier_badge()}  NER:{'✓' if SPACY_READY else '✗'}"
            f"  FT:{CACHE.fulltext_count}"
            f"  FRED:{'✓' if FRED_API_KEY else '✗'}"
            f"  Filings:{CACHE.filing_count}"
            f"  │  ↻ {CACHE.last_refresh}  Arts:{CACHE.article_count}"
        )
        self.query_one("#status_bar2", Static).update(
            f"  CFG:{CONFIG_PATH}"
            f"  │  Z≥{ALERT_ZSCORE_WARN:.1f}σ CRIT≥{ALERT_ZSCORE_CRIT:.1f}σ"
            f"  │  Feeds:{len(RSS_FEEDS)}"
            f"  │  Ctrl+P=command  PIN/UNPIN/NOTE/CHART/SEARCH"
        )
        ...
        self.query_one("#fg_bar", Static).update(
            f"  F/G [{'█'*filled + '░'*(bar_len-filled)}] {fg_val:+d} {fg_label}"
            f"  │  1=Eq 2=FX 3=Bonds 4=Sec 5=Sent 6=FRED"
            f" 7=News 8=Ent 9=Anlyt 0=Alerts F=Filings X=Chart B=Backtest L=Log"
        )
        ...
        parts = [f"SPY:{CACHE.get_price_staleness('SPY')}"]
        for feed in ["rss","newsapi","fred","filings","alerts"]:
            f  = fresh.get(feed, {})
            lb = staleness_label(f.get("last_ok"), warn_s=120, crit_s=600)
            if lb != "✓ live":
                parts.append(f"{feed}:{lb}")
        self.query_one("#freshness_bar", Static).update(
            "  DATA: " + "  ".join(parts)
        )

        # Workspace bar — pinned tickers + recent searches
        pins = WS.pinned_tickers[:6]
        srch = WS.saved_searches[:3]
        pin_s  = "  ".join(f"📌{t}" for t in pins) if pins else ""
        srch_s = "  ".join(f"🔍{s[:20]}" for s in srch) if srch else ""
        self.query_one("#workspace_bar", Static).update(
            f"  WS: {WS.summary()}"
            + (f"  │  {pin_s}" if pin_s else "")
            + (f"  │  {srch_s}" if srch_s else "")
        )
    ...
    def _refresh_chart(self):
        ...
        header = (
            f"\n  ┌─ {_chart_ticker}"
            f"{p['price']:,.4f}" if p and p.get("price") is not None else "—"
            f"  {fmt_chg(p.get('change_pct',0)) if p else '—'}"
            f"  │  Sent1h:{fmt_score(s1)}  24h:{fmt_score(s24)}"
            f"  Corr:{fmt_corr(corr)}"
            f"  Lag:{ll.get('best_lag','?')}h"
            f"  │  {bay.get('signal_str','—')[:22]}\n"
        )
        if note:
            header += f"  📝 {note[:70]}\n"
    ...
    def _refresh_log(self):
        ...
        for key, label in [
            ...
            f    = fresh.get(key,{})
            t.add_row(label,"✓ OK" if n==0 else f"✗ {n}",
                      (f.get("last_ok") or "—")[:19],
    ...
    def action_open_cfg(self):
        self.notify(f"Config: {CONFIG_PATH}\n"
                    f"Workspace: {__import__('market_terminal.storage.workspace', fromlist=['WORKSPACE_PATH']).WORKSPACE_PATH}\n"
    ...
    def action_show_help(self):
        self.notify(
            ...
            "  QUOTE <T>    CHART <T>    NEWS <T>    FILING <T>\n"
            "  SEARCH <q>   PIN <T>      UNPIN <T>   WATCH <T>\n"
            "  NOTE <T> <text>\n\n"
            "TABS: 1=Eq 2=FX 3=Bonds 4=Sec 5=Sent 6=FRED\n"
            "      7=News 8=Ent 9=Anlyt 0=Alerts\n"
            "      F=Filings X=Chart B=Backtest L=Log\n\n"
            "v0.10 new:\n"
            "  Workspace persistence — tab, chart, pins, notes, searches\n"
            "  Event type classifier — EARN/M&A/MACRO/REG/etc on every row\n"
            "  Signal accuracy (B tab) — 1h/4h/24h directional accuracy\n"
            "  PIN / UNPIN / NOTE / WATCH commands\n"
            "  Concurrent RSS (ThreadPoolExecutor — parallel feed fetching)\n"
            "  Retry + backoff on all HTTP calls\n"
            "  Rate limiter on RSS/API/EDGAR requests\n"
            "  Instruments table seeded from watchlist\n",
            title="ESM v0.10  Help", timeout=30)
