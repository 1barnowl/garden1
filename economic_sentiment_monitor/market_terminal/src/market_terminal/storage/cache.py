"""
market_terminal/storage/cache.py
Shared in-memory DataCache — single instance imported by all modules.
v0.9 adds: filings cache, search results, data freshness snapshot.
"""
...
from market_terminal.storage.database import get_db, get_freshness
...
class DataCache:
    def __init__(self):
        ...
        self.fred_data: list    = []
        self.filings: list      = []          # NEW: recent SEC filings
        self.freshness: dict    = {}          # NEW: feed health snapshot
        self.alerts: deque      = deque(maxlen=100)
        self.last_refresh       = "—"
        self.article_count      = 0
        self.fulltext_count     = 0
        self.filing_count       = 0           # NEW
    ...
    def update_filings(self):
        with get_db() as db:
            rows = db.execute("""
                SELECT id, ticker, company, form_type, filed_at,
                       description, sentiment, url
                FROM filings
                ORDER BY filed_at DESC, ingested DESC
                LIMIT 150
            """).fetchall()
            count = db.execute("SELECT COUNT(*) FROM filings").fetchone()[0]
        with self._lock:
            self.filings = [
                {"id": r[0], "ticker": r[1], "company": r[2],
                 "form_type": r[3], "filed_at": r[4],
                 "description": r[5], "sentiment": r[6], "url": r[7]}
                for r in rows
            ]
            self.filing_count = count or 0
    ...
    def get_price_staleness(self, ticker: str) -> str:
        """Return a staleness label for a ticker's last price update."""
        from market_terminal.core.utils import staleness_label
        p = self.get_price(ticker)
        return staleness_label(p.get("ts"))
